...
def calDistance(model):
    for i in range(len(model.demand_id_list)):
        ...
        for j in range(i+1,len(model.demand_id_list)):
            ...
            dist=geodesic((model.demand_dict[from_node_id].y_coord, model.demand_dict[from_node_id].x_coord
                           ), (model.demand_dict[to_node_id].y_coord, model.demand_dict[to_node_id].x_coord)).m
...
if __name__=='__main__':
    demand_file='需求点数据.csv'
    depot_file='配送中心数据.csv'
    run(demand_file,depot_file,epochs=100,pc=0.6,pm=0.2,popsize=100,n_select=80,v_cap=4,opt_type=1)