...
#            particle_dir[i] = self.w * particle_dir[i] + self.c1 * random.random() * (pbest_parameters[i] - particle_loc[i]) + self.c2 * random.random() * (gbest_parameter - particle_dir[i])
            if np.exp(fitness_value[i]) / np.exp(mean_fit) > random.random():
                particle_loc[i] = list(np.array(particle_loc[i]) * w + (1-w) * np.array(particle_dir[i]) + np.array(gbest_parameter))
            else:
...
## 2.5 主函数        
    def main(self):
        ...
        ### 3.3 更新fitness_value
            ...
            particle_loc,particle_dir = self.updata(i+1,particle_loc,particle_dir,gbest_parameter,mbest_parameter,pbest_parameters, fitness_value)
        ## 4.结果展示
        results.sort()
        self.plot(results)
        print(particle_loc)
        sg.cprint('Final parameters are :',gbest_parameter)

           
#图形化界面
layout=[
        # [sg.T('choose data:',size=(18,1)),sg.In(),sg.FolderBrowse()],
        [sg.T('learning data:',size=(18,1)),sg.B('建立规则库')],
        [sg.ML(size=(80,15),reroute_cprint=True)]
    ]#上方单元框
window=sg.Window('石油天然气工控安全防护白名单自学习系统',layout)#标题
#功能执行
while True:
    ...
    if event == "建立规则库":
        if __name__ == '__main__':
            ...
            min_value = 0.001
            #----------------PSO_RBF_SVM-----------------
            pso = MPSO(particle_num,particle_dim,iter_num,c1,c2,w_max,w_min,max_value,min_value)
            pso.main()

    # #建立数据库连接
    # conn = pymysql.connect(host='127.0.0.1',port=3306,
    #         user='root',password='990723',
    #         database='ics',charset='utf8mb4')
    # try:
    #     #获取游标对象
    #     with conn.cursor() as cursor:
    #         #通过游标对象发送SQL数据
    #         affected_rows = cursor.execute(
    #             "insert into 'tb_white' ('pass','value') values('%s','%d')",(trainX,trainY)
    #         )
    #         if affected_rows == 1:
    #             print("提交成功")
    #     #提交事务
