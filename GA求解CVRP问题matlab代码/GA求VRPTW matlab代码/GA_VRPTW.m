...
c101=xlsread('服务时间窗.xlsx');
cap=25;                                                        %车辆最大装载量
cav=50;                                                        %车辆最大装载体积
first_price=850;                                               %车辆起步价
next_price=180;                                                %车辆后续价
%% 提取数据信息
E=c101(1,6);                                                    %配送中心时间窗开始时间
L=c101(1,7);                                                    %配送中心时间窗结束时间
vertexs=c101(:,2:3);                                            %所有点的坐标x和y
customer=vertexs(2:end,:);                                       %顾客坐标
cusnum=size(customer,1);                                         %顾客数
v_num=25;                                                        %车辆最多使用数目
demands_w=c101(2:end,4);                                         %需求量
demands_v=c101(2:end,5);
a=c101(2:end,6);                                                %顾客时间窗开始时间[a[i],b[i]]
b=c101(2:end,7);                                                %顾客时间窗结束时间[a[i],b[i]]
s=c101(2:end,8);                                                %客户点的服务时间
h=pdist(vertexs);
dist=squareform(h);                                             %距离矩阵，满足三角关系，暂用距离表示花费c[i][j]=dist[i][j]
%% 遗传算法参数设置
alpha=100;                                                       %违反的容量约束的惩罚函数系数
belta=1000;                                                      %违反时间窗约束的惩罚函数系数
NIND=100;                                                       %种群大小
MAXGEN=100;                                                     %迭代次数
Pc=0.9;                                                         %交叉概率
Pm=0.05;                                                        %变异概率
...
%% 初始化种群
init_vc=init(cusnum,a,demands_w,demands_v,cap,cav);                             %构造初始解
...
xlabel('代数')
ylabel('最优值')
ObjV=calObj(Chrom,cusnum,cap,cav,demands_w,demands_v,a,b,L,s,dist,alpha,belta,first_price,next_price);             %计算种群目标函数值
preObjV=min(ObjV);
while gen<=MAXGEN
    %% 计算适应度
    ObjV=calObj(Chrom,cusnum,cap,cav,demands_w,demands_v,a,b,L,s,dist,alpha,belta,first_price,next_price);             %计算种群目标函数值
