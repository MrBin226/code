...
function [fv,fviv,fvip,fvC]=farthestINS(removed,rfvc,L,a,b,s,dist,demands_w,demands_v,cap,cav )
nr=length(removed);                   %被移出的顾客的数量
outcome=zeros(nr,3);
for i=1:nr
    %[车辆序号 插入点序号 距离增量]
    [civ,cip,C]= cheapestIP( removed(i),rfvc,L,a,b,s,dist,demands_w,demands_v,cap,cav);
    outcome(i,1)=civ;
    outcome(i,2)=cip;
    outcome(i,3)=C;
end
