...
function [civ,cip,C]= cheapestIP( rv,rfvc,L,a,b,s,dist,demands_w,demands_v,cap,cav)
...
for i=1:NV
    ...
    for j=1:len+1
        ...
        if j==1
            ...
            [Ld,Lv]=leave_load( temp_r,demands_w,demands_v);
            %如果同时满足时间窗约束和容量约束，则该插入点合理，并记录下来
            if isempty(vTW)&&(back<=L)&&(Ld<=cap)&&(Lv<=cav)
