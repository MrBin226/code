...
function flag=Judge(VC,cap,cav,demands_w,demands_v,a,b,L,s,dist)
flag=1;                         %假设满足约束
NV=size(VC,1);                  %车辆使用数目
%% 计算每辆车的装载量
[init_w,init_v]=vehicle_load(VC,demands_w,demands_v);
...
for i=1:NV
    ...
    if init_w(i)>cap || ~isempty(find1) || init_v(i)>cav
        flag=0;
        break
    end
end
