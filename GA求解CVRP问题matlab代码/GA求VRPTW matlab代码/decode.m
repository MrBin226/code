...
function [VC,NV,TD,violate_num,violate_cus]=decode(chrom,cusnum,cap,cav,demands_w,demands_v,a,b,L,s,dist)
...
for j=1:NV
    ...
    route{1}=VC{j};
    flag=Judge(route,cap,cav,demands_w,demands_v,a,b,L,s,dist);     %判断当前方案是否满足时间窗约束和载重量约束，0表示违反约束，1表示满足全部约束
    if flag==0
