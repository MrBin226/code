...
%输入demands  每个顾客的需求量
%输入cap      车辆最大载货量
function [init_vc] = init(cusnum,a,demands_w,demands_v,cap,cav)
...
load_v=0;       %初始路径上在仓库的装载体积为0
i=1;
while i<=cusnum
    %如果没有超过容量约束，则按照左时间窗大小，将顾客添加到当前路径
    if load+demands_w(seq(i))<=cap && load_v+demands_v(seq(i))<=cav        
        load=load+demands_w(seq(i));          %初始在仓库的装载量增加
        load_v=load_v+demands_v(seq(i));      %初始在仓库的装载体积增加
