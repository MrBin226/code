...
for k=1:37
    if zhilu(k)==1
        m=A(k,1)+1;
        n=A(k,2)+1;
        Y(m,m)=Y(m,m)+1/(A(k,3)+1i*A(k,4));
        Y(n,n)=Y(n,n)+1/(A(k,3)+1i*A(k,4));
...
for k=1:37
    if zhilu(k)==1
          %ws(k)=real((u(A(k,1)+1)*u(A(k,1)+1)-u(A(k,1)+1)*u(A(k,2)+1)*(cos(delt(A(k,1)+1)-delt(A(k,2)+1))+sin(delt(A(k,1)+1)-delt(A(k,2)+1))*i)-u(A(k,1)+1)*u(A(k,2)+1)*(cos(delt(A(k,2)+1)-delt(A(k,1)+1))+sin(delt(A(k,2)+1)-delt(A(k,1)+1))*i)+u(A(k,2)+1)*u(A(k,2)+1))*conj(-Y(A(k,1)+1,A(k,2)+1)));    
          %I(k)=(u(A(k,1)+1)*cos(delt(A(k,1)+1))+u(A(k,1)+1)*sin(delt(A(k,1)+1))*i-u(A(k,2)+1)*cos(delt(A(k,2)+1))-u(A(k,2)+1)*sin(delt(A(k,2)+1))*i)*(1/(A(k,3)+A(k,4)*i));
          I(k)=(u(A(k,1)+1)-u(A(k,2)+1))*(1/(A(k,3)+A(k,4)*i));  %忽略相角
          ws(k)=I(k)*conj(I(k))*A(k,3);
    end
end 
%输出各支路电流
I;
