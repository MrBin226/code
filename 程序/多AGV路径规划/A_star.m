...
wait_time=0;
video=VideoWriter('simulation','MPEG-4');
video.FrameRate=2;
open(video);
...
%                     path_opt=[path_opt,(1:size(path_opt,1))'+t];
    ...
    for j=1:AGV_num
        ...
        if ~isempty(temp)
            ...
            if temp(5)==1
                ...
                rectangle('Position',[dynamic_agv_pos(j,1)+0.25,dynamic_agv_pos(j,2)+0.25,0.5,0.5]);
            end
        else
            rectangle('Position',[dynamic_agv_pos(j,1)+0.25,dynamic_agv_pos(j,2)+0.25,0.5,0.5], 'FaceColor',cmap1(j,:));
        end
    ...
    pause(0.1);
    dynamic_path=dynamic_path(dynamic_path(:,3)>=time,:);
end
close(video);
