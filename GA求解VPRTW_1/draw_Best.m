...
C=linspecer(NV);
for i=1:NV
    part_seq=VC{i};            %每辆车所经过的客户
    len=length(part_seq);      %每辆车所经过的客户数量
    for j=0:len
        %当j=0时，车辆从配送中心出发到达该路径上的第一个客户
        if j==0
...
for k=1:size(customer,1)
    text(customer(k,1)+0.5,customer(k,2)+0.5,num2str(k),'color','b','FontSize',10)
end
